...
SPEXTargetLowering::SPEXTargetLowering(const SPEXTargetMachine &TM,
  ...
  setOperationAction(ISD::SELECT_CC, MVT::i16, Legal);
...
static unsigned getCmpRR(unsigned Bits) {
  switch (Bits) {
  case 8:
    return SPEX::CMP8_R;
  case 16:
...
static unsigned getMovRR(unsigned Bits) {
  switch (Bits) {
  case 8:
    return SPEX::MOV8_R;
  case 16:
...
SPEXTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  const SPEXInstrInfo &TII = *ST.getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();

  auto splitMBB = [&](const char *Name) -> MachineBasicBlock * {
    MachineBasicBlock *NewBB = MF.CreateMachineBasicBlock(MBB->getBasicBlock());
    MF.insert(std::next(MBB->getIterator()), NewBB);
    return NewBB;
  };

  auto insertJmp = [&](MachineBasicBlock *From, MachineBasicBlock *To) {
    BuildMI(*From, DL, TII.get(SPEX::JMP)).addMBB(To);
  };

  auto insertBcc = [&](MachineBasicBlock *From, unsigned BccOpc,
                       MachineBasicBlock *To) {
    BuildMI(*From, DL, TII.get(BccOpc)).addMBB(To);
  };

  auto getBccFromCond = [&](ISD::CondCode CC) -> unsigned {
    switch (CC) {
    case ISD::SETEQ:
    ...
    default:
      report_fatal_error("SPEX: unsupported CondCode in custom inserter");
    }
  };

  auto getImmCC = [&]() -> ISD::CondCode {
    int64_t CCImm = MI.getOperand(MI.getNumOperands() - 1).getImm();
    return static_cast<ISD::CondCode>(CCImm);
  };
  ...
  case SPEX::PSEUDO_SETCC64: {
    unsigned Bits = (MI.getOpcode() == SPEX::PSEUDO_SETCC8)    ? 8
                    : (MI.getOpcode() == SPEX::PSEUDO_SETCC16) ? 16
                    : (MI.getOpcode() == SPEX::PSEUDO_SETCC32) ? 32
                                                               : 64;

    Register Dst = MI.getOperand(0).getReg();
    Register LHS = MI.getOperand(1).getReg();
    Register RHS = MI.getOperand(2).getReg();
    ISD::CondCode CC = getImmCC();
    unsigned BccOpc = getBccFromCond(CC);
    unsigned CmpOpc = getCmpRR(Bits);
    unsigned MovOpc = getMovRR(64);

    MachineBasicBlock *TrueBB = splitMBB("setcc.true");
    MachineBasicBlock *EndBB = splitMBB("setcc.end");

    TrueBB->splice(TrueBB->end(), MBB, std::next(MI.getIterator()), MBB->end());
    TrueBB->transferSuccessorsAndUpdatePHIs(MBB);

    MBB->addSuccessor(TrueBB);
    MBB->addSuccessor(EndBB);
    TrueBB->addSuccessor(EndBB);

    BuildMI(*MBB, DL, TII.get(MovOpc), SPEX::RX).addReg(LHS);
    BuildMI(*MBB, DL, TII.get(CmpOpc)).addReg(RHS);
    insertBcc(MBB, BccOpc, TrueBB);

    BuildMI(*MBB, DL, TII.get(SPEX::PSEUDO_LI64), Dst).addImm(0);
    insertJmp(MBB, EndBB);

    BuildMI(*TrueBB, DL, TII.get(SPEX::PSEUDO_LI64), Dst).addImm(1);
    insertJmp(TrueBB, EndBB);
  ...
  case SPEX::PSEUDO_SELECT64: {
    unsigned Bits = (MI.getOpcode() == SPEX::PSEUDO_SELECT8)    ? 8
                    : (MI.getOpcode() == SPEX::PSEUDO_SELECT16) ? 16
                    : (MI.getOpcode() == SPEX::PSEUDO_SELECT32) ? 32
                                                                : 64;

    Register Dst = MI.getOperand(0).getReg();
    Register Cond = MI.getOperand(1).getReg();
    Register TV = MI.getOperand(2).getReg();
    Register FV = MI.getOperand(3).getReg();

    unsigned MovOpc = getMovRR(Bits);
    unsigned CmpOpc = SPEX::CMP64_I32;

    MachineBasicBlock *FalseBB = splitMBB("sel.false");
    MachineBasicBlock *EndBB = splitMBB("sel.end");

    FalseBB->splice(FalseBB->end(), MBB, std::next(MI.getIterator()),
                    MBB->end());
    FalseBB->transferSuccessorsAndUpdatePHIs(MBB);

    MBB->addSuccessor(FalseBB);
    MBB->addSuccessor(EndBB);
    FalseBB->addSuccessor(EndBB);

    BuildMI(*MBB, DL, TII.get(SPEX::MOV64_R), SPEX::RX).addReg(Cond);
    BuildMI(*MBB, DL, TII.get(CmpOpc)).addImm(0);
    insertBcc(MBB, SPEX::BCC_eq_I64, FalseBB);

    BuildMI(*MBB, DL, TII.get(MovOpc), Dst).addReg(TV);
    insertJmp(MBB, EndBB);

    BuildMI(*FalseBB, DL, TII.get(MovOpc), Dst).addReg(FV);
    insertJmp(FalseBB, EndBB);
  ...
  case SPEX::PSEUDO_SELECT_CC64: {
    unsigned Bits = (MI.getOpcode() == SPEX::PSEUDO_SELECT_CC8)    ? 8
                    : (MI.getOpcode() == SPEX::PSEUDO_SELECT_CC16) ? 16
                    : (MI.getOpcode() == SPEX::PSEUDO_SELECT_CC32) ? 32
                                                                   : 64;

    Register Dst = MI.getOperand(0).getReg();
    Register LHS = MI.getOperand(1).getReg();
    Register RHS = MI.getOperand(2).getReg();
    Register TV = MI.getOperand(3).getReg();
    Register FV = MI.getOperand(4).getReg();
    ISD::CondCode CC = getImmCC();
    ...
    MachineBasicBlock *TrueBB = splitMBB("selcc.true");
    MachineBasicBlock *EndBB = splitMBB("selcc.end");
    ...
    BuildMI(*MBB, DL, TII.get(SPEX::MOV64_R), SPEX::RX).addReg(LHS);
    BuildMI(*MBB, DL, TII.get(CmpOpc)).addReg(RHS);
    insertBcc(MBB, BccOpc, TrueBB);

    BuildMI(*MBB, DL, TII.get(MovOpc), Dst).addReg(FV);
    insertJmp(MBB, EndBB);

    BuildMI(*TrueBB, DL, TII.get(MovOpc), Dst).addReg(TV);
    insertJmp(TrueBB, EndBB);
